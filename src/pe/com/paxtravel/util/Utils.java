...
public class Utils {
	...
	private static void mapearBean(Object generico, HttpServletRequest request)
			throws Exception {
			...
			if (!isEmpty(field)) {
				try {
					...
					if (equiv(field.getType(), Long.class)) {
						...
						valor = toDouble(!isEmpty(value[0]) ? value[0]
								.replaceAll("\\,", "") : value[0]);
					} else if (equiv(field.getType(), Float.class)) {
						valor = toFloat(!isEmpty(value[0]) ? value[0]
								.replaceAll("\\,", "") : value[0]);
						...
						// esto xEjemplo: cuando sean varios checkbox con el
						// mismo nombre
						valor = value;
					} else {
						valor = value[0];
	...
	/**
	 * Verifica si un objecto es vacio: - Object: Verifica si es nulo (o vacio
	 * de ser List) - String: El valor es nulo o vacio
	 * 
	 * @author jjurado
	 * @since 27.08.2015
	 * @param object
	...
	public static boolean equiv(Object object1, Object object2) {
		if (isEmpty(object1) && !isEmpty(object2) || !isEmpty(object1)
				&& isEmpty(object2)) {
			return false;
		}
		...
		if (object1 instanceof String && object2 instanceof String) {
			return toBlank(object1.toString()).equals(
					toBlank(object2.toString()));
		}
	...
	public static String dateUtilToStringDDMMYYYY(Date date) {
		...
		String formatedDate = diaStr + "/" + (mesStr) + "/"
	...
	public static String dateUtilToStringYYYYMMDD(Date date) {
		...
		String formatedDate = cal.get(Calendar.YEAR) + "/" + (mesStr) + "/"
				+ diaStr + "/";
		return formatedDate;
	}

	// redondea al nro de decimales establecido sin el problema de la coma
	// flotante/double de Java
	public static BigDecimal roundBigDecimal(double numero, int nroDecimales) {
		return new BigDecimal(numero + "").setScale(nroDecimales,
