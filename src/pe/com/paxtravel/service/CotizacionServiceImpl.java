...
public class CotizacionServiceImpl implements CotizacionService {
	...
	@Override
	public List<CotizacionServicioBean> listarCotizacionServicio(
	...
	public List<FareInfoBean> listarTickets(String cadenaVuelo) {
		...
		try {
			...
			String input = "{\"origin\":\"" + origen + "\",\"destination\":\""
					+ destino + "\",\"date\":\"" + fechaPartida + "\"}";
			...
			while ((output = br.readLine()) != null) {
				...
				final Type listaFareInfo = new TypeToken<List<FareInfoBean>>() {
				}.getType();
				...
				for (FareInfoBean item : fareInfoList) {
					 ...
					 * item.getAirlineCode()); System.out.println("fare? " +
					 * item.getFare()); System.out.println("href? " +
					 * item.getHref()); System.out.println("item? " +
					 * item.toString());
					 */
					...
					System.out.println("fareInfoBean ? getComision "
							+ fareInfoBean.getComision());
					System.out.println("fareInfoBean ? getNombreProveedor "
							+ fareInfoBean.getNombreProveedor());
					System.out.println("fareInfoBean ? getNombreAerolinea "
							+ fareInfoBean.getNombreAerolinea());
					System.out.println("fareInfoBean ? getIdProveedor "
							+ fareInfoBean.getIdProveedor());
					System.out.println("fareInfoBean ? getIdAerolinea "
							+ fareInfoBean.getIdAerolinea());

					/*
					 * pad.comision, p.nombre as proveedor, a.nombre as
					 * aerolinea, p.id_proveedor, a.id_aerolinea <result
					 * property="comision" column="comision" /> <result
					 * property="nombreProveedor" column="proveedor" /> <result
					 * property="nombreAerolinea" column="aerolinea" /> <result
					 * property="idProveedor" column="id_proveedor" /> <result
					 * property="idAerolinea" column="id_aerolinea" />
					 */

					if (fareInfoBean != null) {

						item.setComision(fareInfoBean.getComision());
						item.setNombreProveedor(fareInfoBean
								.getNombreProveedor());
						item.setNombreAerolinea(fareInfoBean
								.getNombreAerolinea());
						...
						System.out.println("item comision? "
								+ item.getComision());
						System.out.println("item proveedor? "
								+ item.getNombreProveedor());
						System.out.println("item aerolinea? "
								+ item.getNombreAerolinea());
						System.out.println("fareinfobean toString? "
								+ item.toString());
					...
					if (encontro) {
						...
						System.out
								.println("Encontro el consolidador: lista Agregada");
					}
	...
	public OrdenPlanificacionBean minorCostTicket(
		...
		if (listaTickets.size() > 0) {
			...
			ordenPlanificacionBean.setIdAerolinea(listaTickets.get(0)
					.getIdAerolinea());
			ordenPlanificacionBean.setIdProveedorAerolinea(listaTickets.get(0)
					.getIdProveedor());
			ordenPlanificacionBean.setPrecioAerolinea(precioMenor);
			ordenPlanificacionBean.setNombreAerolinea(listaTickets.get(0)
					.getNombreAerolinea());
			ordenPlanificacionBean.setComision(Integer.parseInt(listaTickets
					.get(0).getComision()));
			...
			for (FareInfoBean info : listaTickets) {
				...
				if (Double.parseDouble(info.getFare()) < precioMenor) {
					...
					ordenPlanificacionBean
							.setIdAerolinea(info.getIdAerolinea());
					ordenPlanificacionBean.setIdProveedorAerolinea(info
							.getIdProveedor());
					ordenPlanificacionBean.setPrecioAerolinea(precioMenor);
					ordenPlanificacionBean.setNombreAerolinea(info
							.getNombreAerolinea());
					ordenPlanificacionBean.setComision(Integer.parseInt(info
							.getComision()));
	...
	public List<CotizacionDetalleDestinosBean> listarCotizacionPaqueteDestinos(
			int cotizacionId) {
		return cotizacionDAO.listarCotizacionPaqueteDestinos(cotizacionId);
	}
	...
	// excel.xlsRow();
	// k = 1;
	// excel.xlsCell(k++, "No se encontraron resultados para la b\u00FAsqueda",
	// _styleBloque);
	// excel.xlsRow();
	// excel.xlsRow();
	// excel.xlsRow();
