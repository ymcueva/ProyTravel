...
<head>
	...
	<spring:url value="/resources/bootstrap/3.3.2/plugins/datatables-1.10.7/media/css/jquery.dataTables.css" var="dtcss" />
	<spring:url value="/resources/bootstrap/3.3.2/plugins/datatables-1.10.7/extensions/Responsive/css/dataTables.responsive.css" var="respcss" />
	<spring:url value="/resources/bootstrap/3.3.2/plugins/datatables-1.10.7/extensions/Scroller/css/dataTables.scroller.css" var="dtscrollcss" />
	...
	<link  href="${respcss}" rel="stylesheet">
	<link  href="${dtscrollcss}" rel="stylesheet">
		...
		function cargarListaEmpleado() {
			...
			$.ajax({
				...
				success : function(response) {
					...
					var oTable = $('#tblOperacionesMasivo').dataTable();
					oTable.fnDestroy();
						
					$('#tblOperacionesMasivo').dataTable( {
						...
						"info"				: false,
						"columns": [
							{ "data": "intCodFedatario", "class":"left fec01" },
							{ "data": "intDescTipOperativo", "class":"left fec01" },
							{ "data": "intFecInterv", "class":"left fec01","sType": "date-eu" },
							{ "data": "intDescResInterv", "class":"left fec01" },
							{ "data": "intNroActa", "class":"left fec01" }
						]
					} );
					
					//mostrarListaOperacionesMasivo(response.resultado);
					//cantSECIII_3_REP=cantSECIII_3;
...
<body>
	...
	<a href="cargaRegistroEmpleado" >REGISTRAR EMPLEADO</a>
	
	<a href="javascript:;" onclick="cargarListaEmpleado()" >CAGAR EMPLEADOS POR BOOTSTRAP</a>
	...
	<table id="tblEmpleados" class="display" cellspacing="0" width="100%" >
		<thead>
			<tr>
				<th>CODIGO</th>
				<th>USUARIO</th>
		...
		<tbody>
			<c:forEach var="fila" items="${dataEmpleado}">
				<tr>
					<td>${fila.codigo}</td>
					<td>${fila.username}</td>
					<td>${fila.paterno}</td>
					<td>${fila.materno}</td>
					<td>${fila.nombre}</td>
					<td><fmt:formatDate value="${fila.fechaNacimiento}" pattern="dd/MM/yyyy" /></td>
					<td>${fila.dni}</td>
				</tr>
			</c:forEach>
		</tbody>
